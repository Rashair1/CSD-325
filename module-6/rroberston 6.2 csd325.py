...
def displayForest(forest):
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            if forest[(x, y)] == TREE:
                ...
                bext.fg('red')
                print(FIRE, end='')
            elif forest[(x, y)] == WATER: #ADDING WATER TO FOREST
                bext.fg('blue')
                print(WATER, end='')
    ...
    lake_size = 15
    for x in range(WIDTH // 2 - lake_size // 2, WIDTH // 2 + lake_size // 2):
        for y in range(HEIGHT // 2 - lake_size // 2, HEIGHT // 2 + lake_size // 2):
            forest[(x, y)] = WATER
